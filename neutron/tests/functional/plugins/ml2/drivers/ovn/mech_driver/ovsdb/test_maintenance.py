...
class TestMaintenance(_TestMaintenanceHelper):
    ...
    def test_add_provnet_ext_id_to_localnet_ports(self):
        ...
        self.assertEqual(
            'physnet2',
            localnet2.external_ids.get(ovn_const.OVN_PROVNET_EXT_ID_KEY))

        # Simulate pre-upgrade OVN: Neutron-owned port missing the marker.
        self.nb_api.db_remove(
            'Logical_Switch_Port', localnet1.uuid, 'external_ids',
            ovn_const.OVN_PROVNET_EXT_ID_KEY).execute(check_error=True)

        localnet1 = self._find_lswitch_port_row_by_name(localnet1_name)
        self.assertNotIn(ovn_const.OVN_PROVNET_EXT_ID_KEY,
        ...
        self.assertEqual(
            'physnet1',
            localnet1.external_ids.get(ovn_const.OVN_PROVNET_EXT_ID_KEY))
        localnet2 = self._find_lswitch_port_row_by_name(localnet2_name)
        self.assertEqual(
            'physnet2',
            localnet2.external_ids.get(ovn_const.OVN_PROVNET_EXT_ID_KEY))
