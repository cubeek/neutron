...
class BGPChassisBridgeTestCase(BgpTestCaseWithIdls):
    def setUp(self):
        ...
        utils.wait_until_true(
            lambda: (self.bgp_bridge._get_bridge_patch_port_ofport()
                     is not None),
            sleep=0.1,
            timeout=5,
    ...
    def test_bridge_ifaces(self):
        expected_names = ['bgp-patch-port', self.fake_nic.name]
        ifaces = self.bgp_bridge.bridge_ifaces()
        observed_names = [iface['name'] for iface in ifaces]
        self.assertCountEqual(expected_names, observed_names)

    def test__get_bridge_ofports_per_type(self):
        for type in ['patch', '']:
            ofports = self.bgp_bridge._get_bridge_ofports_per_type(type)
            self.assertEqual(1, len(ofports))

    def test__get_bridge_patch_port_ofport(self):
        ofport = self.bgp_bridge._get_bridge_patch_port_ofport()
        self.assertGreater(ofport, 0)
    ...
    def test_configure_flows_complete_has_all_expected_flows(self):
        ...
        self.bgp_bridge.lrp_mac = "aa:bb:cc:dd:ee:ff"
