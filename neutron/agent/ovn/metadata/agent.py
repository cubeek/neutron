...
class PortBindingEvent(row_event.RowEvent):
    def __init__(self, metadata_agent):
        ...
        table = 'Port_Binding'
        super().__init__(self.__class__.EVENTS, table, None)
...
class PortBindingUpdatedEvent(PortBindingEvent):
    EVENTS = (PortBindingEvent.ROW_UPDATE,)

    def match_fn(self, event, row, old):
        print("An event")
        if row.type not in OVN_VIF_PORT_TYPES:
            return
        return (self._is_localport_ext_ids_update(row, old) or
                self._is_new_chassis_set(row, old) or
                self._is_chassis_removed(row, old))
    ...
    def _is_chassis_removed(self, row, old):
        self.log_msg = "Port %s in datapath %s unbound from our chassis"
        try:
...
class PortBindingDeletedEvent(PortBindingEvent):
    EVENTS = (PortBindingEvent.ROW_DELETE)
